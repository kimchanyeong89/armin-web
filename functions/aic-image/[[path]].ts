...
export async function onRequest(context: any): Promise<Response> {
  ...
  const upstreamUrl = AIC_IIIF_BASE + path;
  ...
  // Try to fetch upstream. We remove Origin/Referer and send a browser UA.
  // This runs on Cloudflare's edge (not the browser), so it avoids third-party cookie / referrer issues.
