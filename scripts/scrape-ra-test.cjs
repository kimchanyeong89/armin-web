/**
 * Royal Academy 스크래핑 테스트
 * 
 * 변경 사항:
 * 1. 영상/사진 구분해서 터미널에 출력
 * 2. 플레이스홀더는 실패로 처리 (제외 아님)
 * 3. 실패 항목 재시도
 */
...
// 알려진 placeholder 이미지 URL 패턴
const KNOWN_PLACEHOLDER_PATTERNS = [
  'iC3zV1pGjsk_YgaXox-UgkoTOpjFWoa6lHuFjcPdXLDI0cMLo0',
  'g56tQNK9QD9eSxNFSVVfn4nTyP1tiNbej4Zoxk_6D_Jdx-VIxE',
  'IpcRSKiLIBTs8lOp2lkZ_9iwKlr1MUiE_MiQw8wOUu1GVzw7S4',
  // 디버깅에서 발견된 플레이스홀더 패턴
  'CF2Q3mWoWofKnBM2baJ9X0cuMxNrL3HyVO9bcdLbLQsr6TE00r409wTT3UKw',  // 64개 중복
  'E9edzfAYj7r1k0a2HSpX7-a0y4RWybcMLs9v4alPj5IORwjh8uDLO6dwFfQ',   // 중복
  'BxjLrzX_MSPhz3jqiZ-Gc2r1b_p9j5VvqokQMH0CzMe6tp4CLw',             // 중복
];
...
// 작품 상세 정보 스크래핑
async function scrapeArtwork(context, url, index, waitForRealImage = false, timeout = 15000) {
  ...
  try {
    ...
    const data = await page.evaluate((galleryName) => {
      ...
      const placeholderPatterns = [
        'CF2Q3mWoWofKnBM2baJ9X0cuMxNrL3HyVO9bcdLbLQsr6TE00r409wTT3UKw',
        'E9edzfAYj7r1k0a2HSpX7-a0y4RWybcMLs9v4alPj5IORwjh8uDLO6dwFfQ',
        'BxjLrzX_MSPhz3jqiZ-Gc2r1b_p9j5VvqokQMH0CzMe6tp4CLw',
        'iC3zV1pGjsk_YgaXox-UgkoTOpjFWoa6lHuFjcPdXLDI0cMLo0',
        'g56tQNK9QD9eSxNFSVVfn4nTyP1tiNbej4Zoxk_6D_Jdx-VIxE',
        'IpcRSKiLIBTs8lOp2lkZ_9iwKlr1MUiE_MiQw8wOUu1GVzw7S4',
      ];
      ...
      // 실제 이미지가 있으면 첫 번째 사용, 없으면 기존 방식
      const image = realUrls.length > 0 
        ? realUrls[0] + '=w800'
        : (uniqueUrls.length ? uniqueUrls[0] + '=w800' : null);
...
// 1차 병렬 스크래핑
async function scrapeAllArtworks(context, links) {
...
async function retryFailed(context, failed) {
  ...
  async function retryOne(item) {
    for (let attempt = 1; attempt <= 3; attempt++) {
      const result = await scrapeArtwork(context, item.url, item.index, true, 15000);
...
// 대표 이미지 추출
async function getGalleryCoverImage(page) {
  console.log(`\n🖼️  대표 이미지 추출...`);
...
// 결과 저장
function saveResults(results, coverImage, excluded, finalFailed, videos) {
...
// 보고서 출력
function printReport(results, excluded, failed, videos) {
...
async function main() {
  ...
  console.log('🎨 Royal Academy 스크래핑 테스트');
  console.log('   - 영상/사진 구분 출력');
  console.log('   - Placeholder = 실패로 처리');
  console.log('   - 실패 항목 재시도 (3회)');
  console.log('='.repeat(60));
  ...
  // 1. 링크 수집
  const links = await collectLinks(mainPage);
  
  // 2. 대표 이미지 추출
  const coverImage = await getGalleryCoverImage(mainPage);
  
  // 3. 1차 병렬 스크래핑
  const firstPass = await scrapeAllArtworks(context, links);
  
  // 4. 실패 항목 재시도 (placeholder 포함)
  const { recovered, stillFailed, videos: recoveredVideos } = await retryFailed(context, firstPass.failed);
  ...
  for (let round = 1; round <= 2; round++) {
    ...
    // 재시도
    const { recovered: rec, stillFailed: sf, videos: recVideos } = await retryFailed(context, needRetry);
  ...
  // 7. 저장
  saveResults(allSuccess, coverImage, firstPass.excluded, finalFailed, allVideos);
  
  // 8. 보고서
  printReport(allSuccess, firstPass.excluded, finalFailed, allVideos);
  
  const elapsed = (Date.now() - startTime) / 1000;
  console.log(`\n⏱️  총 소요 시간: ${Math.round(elapsed / 60)}분 ${Math.round(elapsed % 60)}초`);
