/**
 * Rijksmuseum Photography Collection Scraper - API 버전
 * 
 * 발견된 API 사용:
...
const OUTPUT_FILE = path.join(__dirname, '../public/data/rijksmuseum-photography-collection.json');
const PROGRESS_FILE = path.join(__dirname, '../downloads/rijksmuseum-photography-progress.json');
const LOG_FILE = path.join(__dirname, '../downloads/rijksmuseum-photography-log.txt');

// Rate limiting
const DELAY_BETWEEN_PAGES = 500;
const DELAY_BETWEEN_DETAILS = 300;

// Photography facet ID
const PHOTOGRAPHY_FACET_ID = '8e3331ec82020c8bd7583aa971f33ca0';
...
function loadProgress() {
  ...
  return { artworks: [], processedNodeIds: [], lastSearchResults: [], lastPage: 1 };
}
...
async function fetchSearchPage(page = 1) {
  ...
  const path = `${SEARCH_API}?language=en&page=${page}&sortingType=Popularity&collectionSearchContext=Art&facets%5B0%5D.id=${PHOTOGRAPHY_FACET_ID}&facets%5B0%5D.nodeRelationType=HasObjectType`;
...
function transformArtwork(item, detail = null) {
  ...
  if (detail) {
    ...
    if (detail.hasObjectType && Array.isArray(detail.hasObjectType) && detail.hasObjectType.length > 0) {
      artwork.category = detail.hasObjectType[0]; // 첫 번째 object type 사용
    }
...
async function main() {
  log('📷 Rijksmuseum Photography Collection Scraper - API 버전');
  log('='.repeat(60));
