/**
 * Musée Picasso Paris - Prints (Estampes) Collection Scraper v2
 * 
 * 피카소 미술관 파리 - 판화/인쇄물 컬렉션 스크래핑
...
const SEARCH_URL = 'https://cep.museepicassoparis.fr/explorer?field_domaine%5Bestampes%5D=estampes&text=&sort-image=1';
...
const PROGRESS_FILE = path.join(PROGRESS_DIR, 'prints-v2-progress.json');
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'picasso-prints-collection.json');
const COLLECTION_NAME = 'Prints Collection (Estampes)';
const DOMAIN = 'Estampes';
const DEFAULT_MEDIUM = 'Estampe';
 ...
 */
async function main() {
  console.log('🎨 Musée Picasso Paris - Prints (Estampes) Collection Scraper v2');
