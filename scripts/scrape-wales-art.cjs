#!/usr/bin/env node
/**
 * National Museum Wales - Art Collection Scraper (Parallel Version)
 */
...
const PROGRESS_FILE = path.join(__dirname, '../downloads/museum-wales-art-progress.json');
...
const log = msg => console.log(`[${timestamp()}] [ART] ${msg}`);
...
async function main() {
  log(`🚀 Starting Art Collection... (${TEST_MODE ? 'TEST' : 'FULL'})`);
