/**
 * Versailles Full Scraper
 *
 * Scrapes ALL items from the Grand Palais RMN search for Versailles (CATEGORY 271490),
 * merges with any existing versailles-collection.json data (by mediaNumber),
 * then deduplicates by normalized title.
 */
...
// ─────────────────────────────────────────────────────────────
async function main() {
  log('🚀 Starting Versailles full scrape');
  log(`📍 URL: ${TARGET_URL}`);
  ...
  try {
    ...
    let sessionPageOffset = 0; // PAGING_SCOPE_1 offset for current session (session's page 2 = global page 2)
    ...
    while (pageNum <= maxPages) {
      ...
      if ((pageNum - 1) > 0 && (pageNum - 1) % SESSION_CHUNK === 0) {
        ...
        sessionPageOffset = pageNum - 1; // Skip this many pages from new session
        log(`   Skipping ${sessionPageOffset} pages to reach page ${pageNum}...`);
        // Navigate quickly through already-seen pages
        for (let skip = 2; skip <= sessionPageOffset; skip++) {
          const skipUrl = `https://images.grandpalaisrmn.fr${sessionBaseUrl}&PAGING_SCOPE_1=${skip}`;
          await safeGoto(page, skipUrl, `skip-${skip}`);
          await delay(300);
        }
        log(`   Fast-forward complete, at page ${pageNum}`);
      }

      log(`📄 Page ${pageNum}: Extracting items...`);

      // Calculate PAGING_SCOPE_1 for current session
      const scopeNum = pageNum - sessionPageOffset;
      const absoluteUrl = `https://images.grandpalaisrmn.fr${sessionBaseUrl}&PAGING_SCOPE_1=${scopeNum}`;
