...
function renderDashboard() {
    ...
    const border = '=======================================================';
    console.log(border);
    console.log('   Multi-Upload Dashboard (5 Museums) - Realtime');
    console.log(border);
    console.log();
    ...
    stats.forEach(st => {
        ...
        const ratio = st.total > 0 ? `(${st.current} / ${st.total})` : '(0 / 0)';
        let pct = 0;
        if (st.total > 0) {
            pct = Math.round((st.current / st.total) * 100);
        }
...
function startScript(index) {
    ...
    proc.stdout.on('data', (data) => {
        ...
        for (const line of lines) {
            ...
            // matches Uploaded 5, Skipped 150, Err 0 (Total 155/1000)
    ...
    proc.on('close', () => {
        ...
        stats[index].current = stats[index].total; // force 100% on close
    });
}
