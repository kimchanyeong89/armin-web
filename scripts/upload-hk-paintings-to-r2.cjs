...
async function run() {
    const filePath = path.join(__dirname, '../public/data', 'hamburger-kunsthalle-paintings.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: hamburger-kunsthalle-paintings.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        const item = data[i];
        let targetUrl = item.imageUrl || item.image_url || item.image || item.thumbnail || "";
        if (!targetUrl || targetUrl.includes("placeholder")) continue;
        if (targetUrl.includes("placeholder")) continue;        
        ...
        const r2Key = `artworks/hamburger-kunsthalle-paintings/${safeId}-image.webp`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   fetchUrl = 'https://online-sammlung.hamburger-kunsthalle.de' + fetchUrl;
                }
                ...
                if (successes % 10 === 0) {
                    ...
                    fs.writeFileSync(filePath, JSON.stringify(raw, null, 2)); // Save progress
                }
            }
        } catch (e) {
            errors++;
