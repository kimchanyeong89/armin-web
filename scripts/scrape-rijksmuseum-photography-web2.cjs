...
const OUTPUT_FILE = path.join(__dirname, '../public/data/rijksmuseum-photography-web2-collection.json');
const PROGRESS_FILE = path.join(__dirname, '../downloads/rijksmuseum-photography-web2-progress.json');
const LOG_FILE = path.join(__dirname, '../downloads/rijksmuseum-photography-web2-log.txt');
...
async function collectArtworkLinks(page, startPage = 1) {
  ...
  while (hasMorePages && artworkLinks.length < TEST_LIMIT) {
    ...
    try {
      ...
      // 페이지가 있고 링크가 있으면 계속 진행
      if (filteredLinks.length === 0) {
        log('더 이상 링크 없음 - 종료');
        hasMorePages = false;
      } else {
        // TEST_LIMIT에 도달하지 않았으면 다음 페이지로
        if (artworkLinks.length < TEST_LIMIT) {
          currentPage++;
...
async function scrapeArtworkDetail(page, url) {
  try {
    ...
    const artwork = await page.evaluate((pageUrl) => {
      ...
      // 설명 텍스트에서도 날짜 찾기 (예: "1894-11 - 1898")
      if (!result.date && !result.year) {
        const descText = document.body.textContent || '';
        // 연도 범위 패턴: "1894-11 - 1898" 또는 "1894 - 1898"
        const rangeMatch = descText.match(/(\d{4})(?:-\d{1,2})?\s*-\s*(\d{4})/);
        if (rangeMatch) {
          const startYear = parseInt(rangeMatch[1], 10);
          const endYear = parseInt(rangeMatch[2], 10);
          if (startYear >= 1000 && startYear <= 2100) {
            result.year = startYear;
            result.date = rangeMatch[0].trim();
          }
        } else {
          // 단일 연도 패턴
          const yearMatch = descText.match(/\b(18|19|20)\d{2}\b/);
          if (yearMatch) {
            const year = parseInt(yearMatch[0], 10);
            if (year >= 1000 && year <= 2100) {
              result.year = year;
              result.date = yearMatch[0];
            }
          }
        }
      }
        ...
        if (img) {
          ...
          if (src && !src.includes('placeholder') && !src.includes('logo')) {
            if (src.includes('micr.io')) {
              ...
              // URL에 ?가 있으면 &를 사용하고, 없으면 ?를 사용
              if (result.thumbnailUrl.includes('?')) {
                result.imageUrl = result.thumbnailUrl.replace(/[?&]w=\d+/, '&w=1200').replace(/[?&]h=\d+/, '&h=1200');
              } else {
                result.imageUrl = result.thumbnailUrl + '?w=1200&h=1200';
              }
            }
...
async function main() {
  log('📷 Rijksmuseum Photography Collection Scraper V2 (Web 2) - 100개 수집');
  log('='.repeat(60));
