...
const PROGRESS_FILE = path.join(__dirname, '../downloads/mam-painting-progress.json');
const FINAL_OUTPUT = path.join(__dirname, '../public/data/mam-painting-collection.json');

const MAX_PAGES = 150;
const PARALLEL_COUNT = 5;
const SAVE_INTERVAL = 30; // 30개마다 저장
    ...
    const data = await page.evaluate((filterTexts) => {
      ...
      for (const img of imgElements) {
        ...
        if (src.includes('images.navigart.fr') && !src.includes('data:image')) {
          // 첫 번째 사이즈 폴더만 /1000/으로 변경 (예: /400/3Y/12/ -> /1000/3Y/12/)
      ...
      for (let i = 0; i < Math.min(lines.length, 20); i++) {
        ...
        if (!medium && /^(Peinture|Huile|Acrylique|Tempera|Gouache|Aquarelle|Encre)/i.test(line)) medium = line;
...
async function scrape() {
  console.log('🎨 MAM Paris - Painting Collection Full Scraper');
