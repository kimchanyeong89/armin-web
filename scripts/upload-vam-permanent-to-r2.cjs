...
async function run() {
    const filePath = path.join(__dirname, '../public/data', 'vam-permanent-exhibitions.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: vam-permanent-exhibitions.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/vam-permanent-exhibitions/${safeId}-image.webp`;
