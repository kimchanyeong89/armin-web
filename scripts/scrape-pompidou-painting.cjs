/**
 * Centre Pompidou Collection Scraper - Painting (Peinture) Section
 * 
...
const SEARCH_URL = 'https://www.centrepompidou.fr/en/recherche/oeuvres?domaineCollection%5B%5D=Peinture&display=Grid';
const OUTPUT_DIR = path.join(__dirname, '../public/data');
const PROGRESS_DIR = path.join(__dirname, '../downloads/pompidou');
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'pompidou-painting-collection.json');
const PROGRESS_FILE = path.join(PROGRESS_DIR, 'painting-scrape-progress.json');
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      return {
        title,
        artist,
        year: cleanText(tableData['creation date'] || tableData['date'] || ''),
        image,
        domain: cleanText(tableData['domain'] || tableData['domaine'] || ''),
        techniques: cleanText(tableData['techniques'] || tableData['technique'] || ''),
        dimensions: cleanText(tableData['dimensions'] || ''),
        inventoryNo: cleanText(tableData['inventory no.'] || tableData['inventory no'] || '')
      };
    });
...
async function main() {
  console.log('🎨 Centre Pompidou Painting Collection Scraper\n');
  ...
  try {
    ...
    // Get total count
    const totalText = await listPage.$eval('body', el => el.textContent).catch(() => '');
    const totalMatch = totalText.match(/(\d[\d\s]*)\s*results/);
    const expectedTotal = totalMatch ? parseInt(totalMatch[1].replace(/\s/g, '')) : 'unknown';
    console.log(`   Expected total: ${expectedTotal} artworks`);
    ...
    for (let i = 0; i < newLinks.length; i += PARALLEL_DETAILS) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        if (result.status === 'fulfilled' && result.value.title && result.value.image) {
          ...
          allArtworks.push({
            id: `pompidou-painting-${artworkId}`,
            title: data.title,
      ...
      if (allArtworks.length % SAVE_INTERVAL < PARALLEL_DETAILS) {
        ...
        console.log(`\n   💾 Progress saved (${allArtworks.length} artworks, ${failCount} failed)`);
      }
    ...
    allArtworks.slice(0, 3).forEach((art, i) => {
      ...
      console.log(`      Year: ${art.year}, Dimensions: ${art.dimensions || 'N/A'}`);
    });
