#!/usr/bin/env node
// Sorolla collection scraper — Wikidata SPARQL → R2.
// Usage:  node scripts/scrape-sorolla.mjs --limit=100   (pilot)
//         node scripts/scrape-sorolla.mjs               (full ~113)
//
// Output: public/data/sorolla-collection{-pilot}.json + R2 uploads under
//         artworks/sorolla-collection{-pilot}/{id}-{hash8}-imageUrl.webp
...
const STEM    = IS_PILOT ? 'sorolla-collection-pilot' : 'sorolla-collection';
...
WHERE {
  { ?work wdt:P276 wd:Q1592523 . } UNION { ?work wdt:P195 wd:Q1592523 . }
  ?work wdt:P18  ?image ;
...
function categoryFor(instances, genres) {
  ...
  return 'painting'; // Sorolla default (>95% of holdings)
}
...
// ---------- main ----------
async function main() {
  console.log(`[sorolla] mode=${IS_PILOT ? 'pilot' : 'full'} limit=${LIMIT ?? '∞'}`);

  // 1. SPARQL
  console.log('[sorolla] SPARQL …');
  const t0 = Date.now();
  const sparql = await sparqlFetch(QUERY);
  const rows = sparql.results.bindings;
  console.log(`[sorolla] SPARQL ok: ${rows.length} rows in ${Date.now() - t0}ms`);
  ...
  console.log(`[sorolla] unique after dedup: ${unique.length}`);
  ...
  async function processOne(r) {
    ...
    try {
      ...
      if (done % 50 === 0 || done === unique.length) {
        console.log(`[sorolla] ${done}/${unique.length}  ok=${artworks.length}  fail=${failed.length}`);
      }
    }
  }
  ...
  const out = {
    ...
    website: 'https://www.cultura.gob.es/msorolla/colecciones.html',
    source: 'Wikidata SPARQL (wdt:P276 wd:Q1592523)',
  ...
  console.log(`\n[sorolla] wrote ${outPath}  (${artworks.length} artworks)`);
  ...
  if (failed.length) {
    ...
    console.log(`[sorolla] failed log: ${failPath} (${failed.length} items)`);
  }
