/**
 * Mauritshuis Collection Scraper
 * 스크래핑: https://www.mauritshuis.nl/en/our-collection
 * 테스트: 100개 작품 수집
 */
  ...
  while (hasMorePages && artworkLinks.length < TEST_LIMIT) {
    ...
    try {
      ...
      const links = await page.evaluate((baseUrl) => {
        const links = [];
        // 작품 카드에서 링크 추출
        const cards = document.querySelectorAll('a[href*="/en/our-collection/"], a[href*="/collection/"]');
        cards.forEach(card => {
          const href = card.getAttribute('href');
          if (href && (href.includes('/our-collection/') || href.includes('/collection/')) && !href.includes('#')) {
      ...
      // 다음 페이지 확인 또는 더 많은 작품 로드
      const hasMore = await page.evaluate(() => {
        const loadMoreBtn = document.querySelector('.load-more, [data-load-more], button[class*="load"], button[class*="more"]');
        const nextBtn = document.querySelector('a[aria-label="Next"], .pagination-next');
        return (loadMoreBtn && loadMoreBtn.offsetParent !== null) || (nextBtn && nextBtn.offsetParent !== null);
      });
...
async function main() {
  log('═══════════════════════════════════════════════════════════════');
  log('  🏛️  Mauritshuis Collection Scraper');
  log('═══════════════════════════════════════════════════════════════');
