...
async function start() {
    const filePath = path.join(__dirname, '../public/data', 'whitney-collection.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: whitney-collection.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/whitney-collection/${safeId}-image.webp`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   if('whitney-collection'.includes('famsf')) fetchUrl = 'https://www.famsf.org' + fetchUrl;
                   else if('whitney-collection'.includes('lacma')) fetchUrl = 'https://collections.lacma.org' + fetchUrl;
