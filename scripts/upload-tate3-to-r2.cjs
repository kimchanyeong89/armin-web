...
async function run() {
    const filePath = path.join(__dirname, '../public/data', 'tate-artworks.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: tate-artworks.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/tate-artworks/${safeId}-image.webp`;
