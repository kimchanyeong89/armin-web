...
const BASE_URL = 'https://www.collezionegalleriaborghese.it/en/collezione/arte-antica';
const OUTPUT_DIR = path.join(__dirname, '../public/data');
const DOWNLOADS_DIR = path.join(__dirname, '../downloads');
const PROGRESS_FILE = path.join(DOWNLOADS_DIR, 'borghese-antica-progress.json');
const OUTPUT_FILE = 'borghese-arte-antica-collection.json';
const SAVE_INTERVAL = 50;

const delay = ms => new Promise(r => setTimeout(r, ms));
const timestamp = () => new Date().toLocaleTimeString('ko-KR', { hour12: false });
const log = msg => console.log(`[${timestamp()}] [BORGHESE-ANTICA] ${msg}`);
...
async function main() {
    ...
    log(`   Rome, Italy - Arte Antica Collection`);
