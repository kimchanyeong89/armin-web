/**
 * Centre Pompidou Collection Scraper - Drawing (Dessin) Section
 */
...
const SEARCH_URL = 'https://www.centrepompidou.fr/en/recherche/oeuvres?withImage=yes&domaineCollection%5B%5D=Dessin&secteurCollection%5B%5D=Cabinet%20d%27art%20graphique&display=Grid';
const OUTPUT_DIR = path.join(__dirname, '../public/data');
const PROGRESS_DIR = path.join(__dirname, '../downloads/pompidou');
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'pompidou-drawing-collection.json');
const PROGRESS_FILE = path.join(PROGRESS_DIR, 'drawing-scrape-progress.json');

// Scraping settings - BALANCED
const MAX_ARTWORKS = 50000;
...
async function main() {
  console.log('✏️ Centre Pompidou Drawing Collection Scraper\n');
  ...
  try {
    ...
    for (let i = 0; i < newLinks.length; i += PARALLEL_DETAILS) {
      ...
      for (let j = 0; j < batch.length; j++) {
        if (results[j].status === 'fulfilled' && results[j].value.title && results[j].value.image) {
          ...
          allArtworks.push({
            id: `pompidou-drawing-${batch[j].split('/').pop()}`,
            title: d.title, artist: d.artist || 'Unknown', year: d.year, image: d.image,
            dimensions: d.dimensions, medium: d.techniques, type: '2D',
            inventoryNo: d.inventoryNo, source: 'Centre Pompidou', collectionArea: 'Drawing', detailUrl: batch[j]
          });
    ...
    fs.writeFileSync(OUTPUT_FILE, JSON.stringify({
      museum: 'Centre Pompidou', museumId: 'centre-pompidou', collectionName: 'Drawing Collection',
