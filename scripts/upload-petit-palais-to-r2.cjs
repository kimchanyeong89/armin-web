/**
 * Upload Petit Palais Collection to R2
...
const INPUT_FILE = path.join(__dirname, '../public/data/petit-palais-collection.json');
const OUTPUT_FILE = INPUT_FILE; // Update in place
const PROGRESS_FILE = path.join(__dirname, '../downloads/petit-palais-upload-progress.json');
...
function downloadImage(url) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const tempFile = `/tmp/petit-palais-temp-${Date.now()}.jpg`;
...
async function main() {
  console.log('🏛️ Petit Palais Paris → R2 Uploader');
  ...
  for (let i = 0; i < objects.length; i++) {
    const obj = objects[i];
    const key = `petit-palais/${obj.id}.webp`;
    ...
    // Check if already R2 URL
    if (obj.image && obj.image.startsWith(R2_PUBLIC_URL)) {
      skipped++;
      continue;
    }
    ...
    try {
      ...
      if (uploaded % 10 === 0) {
        ...
        console.log(`  💾 Progress saved (${uploaded} uploaded)`);
      }
