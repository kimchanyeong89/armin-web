...
async function run() {
    const filePath = path.join(__dirname, '../public/data', 'mucem-collection.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: mucem-collection.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/mucem-collection/${safeId}-image.webp`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   fetchUrl = 'https://mucem.org' + fetchUrl;
                }
