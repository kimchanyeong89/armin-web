...
async function start() {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = \`artworks/${slug}/\${safeId}-image.webp\`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   if('${slug}'.includes('famsf')) fetchUrl = 'https://www.famsf.org' + fetchUrl;
                   else if('${slug}'.includes('lacma')) fetchUrl = 'https://collections.lacma.org' + fetchUrl;
...
targets.forEach(t => {
    fs.writeFileSync(t.script, generateScript(t.file));
});

targets.push({ name: 'NGA', file: 'nga-collection.json', script: 'scripts/upload-nga-to-r2.cjs' });
targets.push({ name: 'Whitney', file: 'whitney-collection.json', script: 'scripts/upload-whitney-to-r2.cjs' });

targets.slice(-2).forEach(t => {
    fs.writeFileSync(t.script, generateScript(t.file));
    fs.chmodSync(t.script, '755');
});
