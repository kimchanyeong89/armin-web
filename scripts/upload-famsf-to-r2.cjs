...
async function start() {
    const filePath = path.join(__dirname, '../public/data', 'famsf-collections.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: famsf-collections.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/famsf-collections/${safeId}-image.webp`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   if('famsf-collections'.includes('famsf')) fetchUrl = 'https://www.famsf.org' + fetchUrl;
                   else if('famsf-collections'.includes('lacma')) fetchUrl = 'https://collections.lacma.org' + fetchUrl;
