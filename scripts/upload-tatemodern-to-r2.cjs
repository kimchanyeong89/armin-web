...
async function run() {
    const filePath = path.join(__dirname, '../public/data', 'tate-modern-collection.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: tate-modern-collection.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/tate-modern-collection/${safeId}-image.webp`;
