/**
 * Rijksmuseum Painting Collection Scraper V2
 * 
 * 전략:
 * 1. 웹사이트 검색 페이지에서 painting 목록 수집 (올바른 URL만 필터링)
    ...
    try {
      ...
      const links = await page.evaluate(() => {
        ...
        cards.forEach(card => {
          ...
          const isArtworkUrl = href.match(/\/en\/collection\/(object\/[^/?]+|SK-[A-Z0-9-]+)/);
    ...
    const artwork = await page.evaluate((pageUrl) => {
      ...
      for (const selector of imageSelectors) {
        ...
        if (img) {
          ...
          if (src && !src.includes('placeholder') && !src.includes('logo') && src.includes('assets')) {
...
async function main() {
  log('🎨 Rijksmuseum Painting Collection Scraper V2 시작');
  log('='.repeat(60));
  ...
  try {
    ...
    const linksToProcess = artworkLinks.slice(startIndex, startIndex + TEST_LIMIT);
    log(`🧪 테스트 모드: ${linksToProcess.length}개만 처리`);
    ...
    for (let i = 0; i < linksToProcess.length; i++) {
      ...
      if (processedUrls.has(url)) {
        log(`⏭️  ${i + 1}/${linksToProcess.length} 이미 처리됨: ${url}`);
        continue;
      }
      
      process.stdout.write(`\r📄 ${i + 1}/${linksToProcess.length} 처리 중...`);
