#!/usr/bin/env node
/**
 * Upload Tate Britain artwork images to Cloudflare R2
 * Input: downloads/tate-britain-artworks.json
 * Output: public/data/tate-britain-artworks.json (with R2 URLs)
 */
...
const INPUT_FILE = path.join(__dirname, '../downloads/tate-britain-artworks.json');
...
async function downloadImage(url) {
  try {
    ...
    // Try fallback image size (_9 instead of _10)
    if (url.includes('_10.jpg')) {
      try {
        const fallbackUrl = url.replace('_10.jpg', '_9.jpg');
        const response = await got(fallbackUrl, {
          responseType: 'buffer',
          timeout: { request: 30000 },
          retry: { limit: 2 },
          headers: {
...
async function processArtwork(artwork, index, total) {
  const accession = (artwork.accession || artwork.id).toLowerCase().replace(/[^a-z0-9-]/g, '-');
  const r2Key = `galleries/tate-britain/artworks/${accession}.jpg`;
  ...
  if (exists) {
    ...
    if ((index + 1) % 50 === 0 || index + 1 === total) {
      console.log(`[${index + 1}/${total}] ⏭ ${artwork.title} (already exists)`);
    }
  ...
  if (!buffer) {
    ...
    return { ...artwork, r2Image: null, failed: true };
  }
