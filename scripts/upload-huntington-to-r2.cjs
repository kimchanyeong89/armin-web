...
async function start() {
    const filePath = path.join(__dirname, '../public/data', 'huntington-collection.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: huntington-collection.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/huntington-collection/${safeId}-image.webp`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   if('huntington-collection'.includes('famsf')) fetchUrl = 'https://www.famsf.org' + fetchUrl;
                   else if('huntington-collection'.includes('lacma')) fetchUrl = 'https://collections.lacma.org' + fetchUrl;
