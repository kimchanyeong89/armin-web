...
async function start() {
    const filePath = path.join(__dirname, '../public/data', 'lacma-classification-22.json');
    if (!fs.existsSync(filePath)) {
        console.log(`File not found: lacma-classification-22.json`);
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const r2Key = `artworks/lacma-classification-22/${safeId}-image.webp`;
        ...
        try {
            ...
            if (exists) {
                ...
                if(fetchUrl.startsWith('/')) {
                   if('lacma-classification-22'.includes('famsf')) fetchUrl = 'https://www.famsf.org' + fetchUrl;
                   else if('lacma-classification-22'.includes('lacma')) fetchUrl = 'https://collections.lacma.org' + fetchUrl;
