...
new_func = """def render_dashboard(state, current_e=None, last_error=None):
    ...
    for e_id, count in sorted_exhibitions:
        ...
        fares = state["museum_failed"].get(e_id, 0)
        
        # Avoid the math overflowing over 100% physically
        percent = min((successes / count) * 100, 100.0)
...
code = re.sub(r'def render_dashboard\(state, current_e=None, last_error=None\):.*?with open\("EMBEDDING_PROGRESS.md", "w"\) as f:\s+f\.write\("\\n"\.join\(lines\)\)', new_func, code, flags=re.DOTALL)
