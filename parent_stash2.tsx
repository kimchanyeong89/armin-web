...
export default function GlobalSearchBar({ onOpenLightbox, onNavigateToMuseum, museums = [], isModalOpen, initialQuery, inlineMode = false }: GlobalSearchBarProps) {
    ...
    // Restore query from sessionStorage if available or use initialQuery
    ...
    // Handle initialQuery or custom event trigger
    useEffect(() => {
        if (initialQuery && initialQuery !== query) {
            setQuery(initialQuery);
            if (initialQuery.length > 0) setIsExpanded(true);
        }
    }, [initialQuery]);
    ...
    useEffect(() => {
        ...
        window.addEventListener('global-search-trigger', handleTrigger);
        return () => window.removeEventListener('global-search-trigger', handleTrigger);
    }, []);
    ...
    const artistGalleryColumns = useMemo(() => {
        ...
        const safeCount = Math.min(desiredCount, Math.max(1, artistGallery.artworks.length));
        const columns = Array.from({ length: safeCount }, () => [] as SearchableArtwork[]);
        artistGallery.artworks.forEach((art, index) => {
            columns[index % safeCount].push(art);
        });
        return columns;
    }, [artistGallery, isMobile]);
    ...
    // Save query to sessionStorage and notify global listeners
    useEffect(() => {
        try {
            if (query) {
                sessionStorage.setItem('globalSearchQuery', query);
                // Dispatch event to sync other search inputs (e.g. Navbar)
                window.dispatchEvent(new CustomEvent('global-search-update', { detail: { query } }));
            } else {
    ...
    const ensureWorker = useCallback(() => {
        ...
        workerRef.current.onmessage = (e) => {
            ...
            const resolvePreciseCollectionId = (art: SearchableArtwork) => {
                const safeId = String(art.id || '');
                // MAMCS Strasbourg Logic
                if (safeId && (safeId.startsWith('mamcs-') || safeId.startsWith('mamcs_'))) {
                    if (safeId.startsWith('mamcs-paintings-')) return 'mamcs-strasbourg-paintings-collection';
                    if (safeId.startsWith('mamcs-drawings-')) return 'mamcs-strasbourg-drawings-collection';
                    if (safeId.startsWith('mamcs-photography-')) return 'mamcs-strasbourg-photography-collection';
                    if (safeId.startsWith('mamcs-graphic-design-')) return 'mamcs-strasbourg-graphic-design-collection';
                }
    ...
    const handleSelectMuseum = useCallback((museum: Museum) => {
        ...
        // Do NOT auto-enter the first permanent exhibition. Just navigate to the museum pin/panel.
        onNavigateToMuseum({ id: museum.id, name: museum.name });
    }, [onNavigateToMuseum]);
        ...
        <>
            ...
            <div
                ...
                style={{
                    ...
                    ...(inlineMode ? {
                        ...
                        background: isExpanded ? 'rgba(255, 255, 255, 0.65)' : 'rgba(255, 255, 255, 0.4)',
                        backdropFilter: 'blur(30px) saturate(200%)',
                        WebkitBackdropFilter: 'blur(30px) saturate(200%)',
                        boxShadow: isExpanded ? '0 8px 32px rgba(0,0,0,0.15)' : '0 4px 15px rgba(0, 0, 0, 0.05), inset 0 1px 1px rgba(255, 255, 255, 0.6)',
                        overflow: 'hidden',
                    }),
            ...
            >
                ...
                <div style={{
                    ...
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={inlineMode && !isExpanded ? "#000" : "#666"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                >
                    ...
                    {isExpanded && isLoading && (
                        <div style={{ padding: '16px', textAlign: 'center', color: '#666', fontSize: 13, borderTop: '1px solid #e5e7eb' }}>
                            Loading search data...
                        </div>
                    )}
            ...
            {artistGallery && createPortal(
                <div
                    style={{
                        ...
                        background: isMobile ? '#fff' : 'rgba(255, 255, 255, 0.65)',
                        backdropFilter: isMobile ? 'none' : 'blur(30px) saturate(200%)',
                        WebkitBackdropFilter: isMobile ? 'none' : 'blur(30px) saturate(200%)',
                        display: 'flex',
                ...
                >
                    <div
                        ...
                        style={isMobile ? {
                            ...
                            display: 'flex',
                            flexDirection: 'column',
                            overflowY: 'auto', // Allow full page scrolling
                            WebkitOverflowScrolling: 'touch',
                            boxShadow: 'none',
                            transform: 'none',
                        } : {
                    ...
                    >
                        {/* Header */}
                        <div
                            style={{ flexShrink: 0, background: isMobile ? '#fff' : 'transparent', cursor: 'auto' }}
                        >

                            <div style={{
                                padding: isMobile ? '16px 20px 0' : '0 0 24px 0',
                                borderBottom: isMobile ? '1px solid #f0f0f0' : '1px solid #ececec',
                                ...
                                <div style={{ flex: 1, width: '100%' }}>
                                    <ArtistWikiPanel
                                        ...
                                        imageUrl={artistGallery.artworks?.[0]?.image
                                            ? getOptimizedImageUrl(artistGallery.artworks[0].image, 320)
                                            : undefined}
                                        ...
                                        headerSlot={
                                            <div>
                                                ...
                                                <p style={{ fontSize: isMobile ? 13 : 15, color: '#666', margin: isMobile ? '4px 0 0' : '10px 0 0' }}>{artistGallery.artworks.length.toLocaleString()} artworks</p>
                                            </div>
                                        }
                                    />
                                </div>
                        ...
                        <div style={{
                            ...
                            padding: isMobile ? '12px' : '10px 0 40px 0'
                        }}>
                            <div style={{
                                display: 'flex',
                                gap: isMobile ? 12 : 24,
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div style={{
                                                        ...
                                                        <img
                                                            ...
                                                            style={{
                                                                ...
                                                                transform: hoveredArtworkId === art.id ? 'scale(1.05)' : 'scale(1)'
                                                            }}
                                                            loading="lazy"
                                                            alt={art.name}
                                                    ...
                                                    <div style={{ marginTop: 8 }}>
                                                        ...
                                                        <div style={{ fontSize: isMobile ? 10 : 12, color: '#888', marginTop: 2 }}>
                                                            {museumDisplay}
                                                        </div>
                                                    </div>
                                                </div>
