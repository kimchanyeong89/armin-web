...
export default function GlobeWithCities({ focusLatLng = null, autorotate = false, stroke = '#2b3138', strokeWidth = 1.5, exhibitions = [], onSelectExhibition }: GlobeWithCitiesProps) {
  ...
  useEffect(() => {
    ...
    console.log(`[GlobeWithCities] UK exhibitions in props: ${ukExhibitions.length}`);
    ...
    const updateLayerVisibility = () => {
      console.log('[GlobeWithCities] Updating layer visibility:', { hasAtlasCountries, hasAtlasStates, hasAdminGeo, hasCityGeo, hasUrbanAreas });
      ...
      if (hasUrbanAreas) {
        ...
        console.log('[GlobeWithCities] Showing urban areas (global city boundaries) - ALWAYS VISIBLE');
      } else if (hasCityGeo) {
        gCities.style('display', 'block');
        gUrbanAreas.style('display', 'none');
        gUrban.style('display', 'none');
        console.log('[GlobeWithCities] Showing local cities');
      } else {
        gCities.style('display', 'none');
        gUrbanAreas.style('display', 'none');
        gUrban.style('display', 'block');
        console.log('[GlobeWithCities] Showing fallback urban rings');
      }
    };
    ...
    try {
      ...
      (window as any).fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (/world-atlas@|cdn\.jsdelivr\.net\/npm\/world-atlas@/i.test(url)) {
          console.warn('[GlobeWithCities] BLOCKED external fetch:', url);
    ...
    (async () => {
      try {
        ...
        console.warn('[GlobeWithCities] Failed to load local countries geojson, drawing sphere only', e);
      }
    })();
    ...
    const loadAdminIfNeeded = async () => {
      if (hasAdminGeo) return;
      try {
        console.log('[GlobeWithCities] trying admin boundaries');
    ...
    // Load global urban areas (city boundary polygons) - ALWAYS LOAD
    const loadUrbanAreas = async () => {
      if (urbanAreasLoadAttempted || hasUrbanAreas) return;
      urbanAreasLoadAttempted = true;
      console.log('[GlobeWithCities] Loading urban areas (ALWAYS)...');
      try {
        // Try existing files first, then package versions
        const urls = ['/atlas/urban-areas-sample.json', '/atlas/urban-areas.json', '/atlas/package/urban-areas-sample.json', '/atlas/package/urban-areas.json'];
        let json: any | null = null;
        for (const url of urls) {
          try {
            ...
            const ct = (res.headers.get('content-type') || '').toLowerCase();
            if (!/json/.test(ct)) {
              console.log(`[GlobeWithCities] Wrong content-type for ${url}: ${ct}`);
              continue;
            }
            json = await res.json();
            console.log(`[GlobeWithCities] Successfully loaded JSON from ${url}, type: ${json.type}`);
            if (json) break;
          } catch (e) {
            console.log(`[GlobeWithCities] Error fetching ${url}:`, e);
          }
        }
        if (!json) {
          console.log('[GlobeWithCities] No urban areas data found');
          return;
        }
        let features: any[] = [];
        ...
        console.log(`[GlobeWithCities] Extracted ${features.length} urban area features`);
        if (!features.length) return;
        gUrbanAreas
          ...
          .attr('d', path as any)
          .style('fill', 'none')
          .style('stroke', '#7c3aed') // 보라색으로 도시 경계 표시
          .style('stroke-width', String(strokeWidth * 0.8))
          .style('vector-effect', 'non-scaling-stroke')
          .style('stroke-linejoin', 'round')
          .style('stroke-linecap', 'round')
          .style('opacity', 0.8);
        hasUrbanAreas = true;
        console.log('[GlobeWithCities] Urban areas rendered successfully - global city boundaries loaded and ALWAYS VISIBLE');
        updateLayerVisibility();
        svg.selectAll('path').attr('d', path as any);
      } catch (e) {
        console.log('[GlobeWithCities] Error loading urban areas:', e);
      }
    };

    // Load urban areas - ALWAYS load for this component
    (async () => {
      try {
        console.log('Loading urban areas data (ALWAYS)...');
        await loadUrbanAreas();
      } catch (e) {
        // eslint-disable-next-line no-console
        console.error('Urban areas loading failed', e);
      }
    })();
